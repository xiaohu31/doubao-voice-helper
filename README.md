...
| 模式 | 操作方式 | 适用场景 | 默认按键 |
|------|----------|----------|----------|
| **按着说** | 按住 → 说话 → 松开自动插入 | 快速短句 | 右Ctrl |
| **自由说** | 点一下开始 → 说话 → 再点结束 | 长段落 | 鼠标侧键1 |
| **按着说+自动发送** | 按住 → 说话 → 松开自动插入并发送 | 快速聊天 | 左Ctrl+左Win |
...
| 配置项 | 说明 | 默认值 |
|--------|------|--------|
| 按着说触发键 | 按住说话的触发按键 | 右Ctrl |
| 自由说触发键 | 点击切换的触发按键 | 鼠标侧键1 |
| 按着说+自动发送触发键 | 按住说话并自动发送的触发按键 | 左Ctrl+左Win |
| 取消键 | 自动发送模式下取消语音输入 | Z |
| 豆包快捷键 | 唤起豆包语音的快捷键 | Ctrl+D |
| 插入延迟 | 松开后等待识别完成的时间 | 300ms |
| 自动发送延迟 | 自动发送前等待内容粘贴完成的时间 | 50ms |
